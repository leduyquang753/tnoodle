...
class ScrambleRequest {
    ...
    static {
        try {
            monoFont = BaseFont.createFont("fonts/consola.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED);
            sansSerifFont = BaseFont.createFont("fonts/segoeui.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED);
    ...
    private static PdfReader createPdf(String globalTitle, Date creationDate, ScrambleRequest scrambleRequest, Locale locale) throws DocumentException, IOException {
        ...
        for(int pageN = 1; pageN <= pr.getNumberOfPages(); pageN++) {
            ...
            switch (pageN % 2) {
                case 0:
                    ColumnText.showTextAligned(cb,
                        Element.ALIGN_LEFT, new Phrase(pageN + "/" + pr.getNumberOfPages()),
                        pageSize.getLeft() + 40, pageSize.getBottom() + 40, 0);
                    ColumnText.showTextAligned(cb,
                        Element.ALIGN_RIGHT, new Phrase(Utils.SDF.format(creationDate)),
                        pageSize.getRight() - 40, pageSize.getBottom() + 40, 0);
                    break;
                case 1:
                    ColumnText.showTextAligned(cb,
                        Element.ALIGN_RIGHT, new Phrase(pageN + "/" + pr.getNumberOfPages()),
...
//      return ps.getReader();
    ...
    solutionLines:
        ...
        cb.showTextAligned(PdfContentByte.ALIGN_LEFT, translate("fmc.wcaid", locale), competitorInfoLeft+padding, top-offsetTop, 0);
